...
INSERT INTO public.seller_info (id, name, avatar, rating, phone, whatsapp)
VALUES (
    uuid_generate_v4(),
...
-- 9. CRIAR USUÁRIO ADMIN PADRÃO (EXECUTE APENAS UMA VEZ)
-- Substitua 'caio@caio.com' pelo seu email
DO $$
BEGIN
    -- Verificar se o usuário admin já existe
    IF NOT EXISTS (SELECT 1 FROM public.users WHERE email = 'caio@caio.com') THEN
        -- Inserir usuário admin (você precisa criar este usuário no Auth primeiro)
    ...
    ELSE
        ...
        UPDATE public.users 
        SET is_admin = true, profile_complete = true, name = 'Caio Admin'
...
SELECT 'Usuários:' as info, count(*) as total FROM public.users;
SELECT 'Veículos:' as info, count(*) as total FROM public.vehicles;
