-- =====================================================
-- POPULAR DADOS DE TESTE (VERSÃO SEGURA)
-- Execute este script para adicionar dados de teste
-- Não remove dados existentes, apenas adiciona novos
-- =====================================================

-- 1. INSERIR VEÍCULOS DE TESTE (apenas se não existirem)
...
BEGIN
    ...
    IF vehicle_count = 0 THEN
        -- Inserir veículos de teste apenas se não existirem
        ...
        RAISE NOTICE 'Veículos de teste inseridos com sucesso!';
    ELSE
        RAISE NOTICE 'Veículos já existem (% veículos encontrados). Pulando inserção.', vehicle_count;
    END IF;
END $$;

-- 2. INSERIR IMAGENS PARA OS VEÍCULOS (apenas se não existirem)
DO $$
DECLARE
    image_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO image_count FROM public.vehicle_images;
    
    IF image_count = 0 THEN
        -- Inserir imagens apenas se não existirem
        ...
        RAISE NOTICE 'Imagens de teste inseridas com sucesso!';
    ELSE
        RAISE NOTICE 'Imagens já existem (% imagens encontradas). Pulando inserção.', image_count;
    END IF;
END $$;

-- 3. INSERIR CARACTERÍSTICAS PARA OS VEÍCULOS (apenas se não existirem)
DO $$
DECLARE
    feature_count INTEGER;
BEGIN
    SELECT COUNT(*) INTO feature_count FROM public.vehicle_features;
    
    IF feature_count = 0 THEN
        -- Inserir características apenas se não existirem
        ...
        RAISE NOTICE 'Características de teste inseridas com sucesso!';
    ELSE
        RAISE NOTICE 'Características já existem (% características encontradas). Pulando inserção.', feature_count;
    END IF;
END $$;
